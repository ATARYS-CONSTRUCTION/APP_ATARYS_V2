...
def analyze_model(model_class):
    ...
    for column_name, column_obj in model_class.__table__.columns.items():
        ...
        if 'Numeric' in column_type:
            ...
            # Remplacer l'affichage FLOAT par REAL
            field_type = 'real'  # Utilisé pour le mapping, affiché comme REAL côté frontend
            field_validation['step'] = '0.00001'
...
if __name__ == "__main__":
    ...
    with app.app_context():
        ...
        for col in schema['columns']:
            info = (
                f"• {col['name']}: {col['type']} ("
                f"{'obligatoire' if col['required'] else 'optionnel'})"
            )
            print(info)
            if col['default']:
